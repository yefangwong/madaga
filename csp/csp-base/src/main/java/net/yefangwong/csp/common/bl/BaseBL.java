...
/**
 * BaseBL - 業務邏輯層 5 大生命週期抽象基類 (DataPipeline 模式)
 *
 ...
 * 內建 AppErrors 診斷容器與 DataPipeline 萬能傳參管道，
 * 無縫對齊 PureMVC Facade 門面與 Spring 聲明式事務管理
 *
 * @param <RESP> 響應 Payload 物件型別 (Response DTO)
 * @author Mark Wong (yefangwong)
 * @since 1.0.0 (2026-07-21)
 */
public abstract class BaseBL<RESP> {
     ...
     *
     * @param pipeline 萬能傳參管道 (DataPipeline)
     * @return true 代表校驗通過；false 代表失敗 (並將錯誤細節推入 errors)
     */
    protected abstract boolean validateInput(DataPipeline pipeline);
     ...
     */
    protected abstract boolean verifyAuthority(DataPipeline pipeline);
     ...
     */
    protected abstract RESP executeBusiness(DataPipeline pipeline) throws Exception;

    /**
     * 4. ISO 27001 審計軌跡寫入 (預設 Hook，子類別可視需求覆寫)
     *
     * @param pipeline 萬能傳參管道 (DataPipeline)
     * @param status   執行狀態 (e.g. "SUCCESS", "FAILED")
     * @param details  詳細審計描述
     * @return true 代表寫入成功；false 代表記錄失敗
     */
    protected boolean writeAuditLog(DataPipeline pipeline, String status, String details) {
        GlobalContext ctx = pipeline != null ? pipeline.getContext() : null;
        String email = ctx != null ? ctx.getOperatorEmail() : "system@madaga.com";
        String repoId = ctx != null ? ctx.getComCode() : "default-repo";
        log.info("[AUDIT-TRAIL] Status: {}, Operator: {}, Target: {}, Details: {}", status, email, repoId, details);
        return true;
    }
    ...
    /**
     * 執行完整 5 大生命週期工序 (DataPipeline 模式)
     *
     * @param pipeline   萬能傳參管道 (含有 GlobalContext 與 Request Payload)
     * @param actionCode 操作動作代碼
     * @return 標準 ApiResult<RESP> 封裝響應
     */
    public ApiResult<RESP> process(DataPipeline pipeline, String actionCode) {
        errors.clear();

        if (pipeline == null) {
            return ApiResult.failure(400, "DataPipeline 不能為空");
        }

        GlobalContext context = pipeline.getContext();
        String operatorEmail = context != null ? context.getOperatorEmail() : "system@madaga.com";

        // 1. 前置輸入防呆
        if (!validateInput(pipeline)) {
        ...
        // 2. 權限校對
        if (!verifyAuthority(pipeline)) {
            String msg = errors.hasErrors() ? errors.getFirstMessage() : "操作權限不足";
            log.warn("Authority verification failed for action [{}], operator [{}]: {}", actionCode, operatorEmail,
                    msg);
        ...
        try {
            // 3. 核心業務邏輯
            RESP response = executeBusiness(pipeline);

            // 4. ISO 27001 審計軌跡
            writeAuditLog(pipeline, "SUCCESS", "Action [" + actionCode + "] executed successfully");

            // 5. 響應封裝
            return ApiResult.success(response);
        } catch (Exception e) {
            log.error("Execution exception in BaseBL process for action [" + actionCode + "]", e);
            writeAuditLog(pipeline, "FAILED: " + e.getMessage(), e.getMessage());
